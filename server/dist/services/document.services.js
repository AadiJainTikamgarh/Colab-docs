...
const createDocumentService = async (title, ownerId, data = {}) => {
    if (!title) {
...
const getUserDocumentService = async (userId) => {
    if (!userId) {
...
const getDocumentByIdService = async (userId, docId) => {
    if (!userId || !docId) {
...
const updateDocumentService = async (userId, docId, data) => {
    if (!userId || !docId) {
        throw new ApiError(404, "All details are required");
    }
...
const addCollaborationService = async (userId, docId, collaboratorEmail, role) => {
...
const deleteDocumentService = async (userId, docId) => {
    if (!userId || !docId) {
        throw new ApiError(404, "All fields are required");
    }
    await documents.findOneAndDelete({ owner: userId, _id: docId });
};
const removeDocumentCollaborationService = async (userId, docId, collaboratorId) => {
    ...
    const docs = await documents.findOneAndUpdate({ owner: userId, _id: docId }, { $pull: { collaborators: { userId: collaboratorId } } });
    return { docs };
};
