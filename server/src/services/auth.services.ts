...
const verifyEmailService = async (unhashedToken: string) => {
  if (!unhashedToken) {
  ...
  if ((user.emailVerificationTokenExpiry as Date) < new Date(Date.now())) {
...
const resendEmailVerificationMailService = async (userId: string) => {
  if (!userId) {
...
const forgotPasswordRequestService = async (
